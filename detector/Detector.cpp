...
bool Detector::RecognizeFace(const cv::Mat& frame, const std::shared_ptr<char>& pImgBuf, long bufLen, float& score)
{
	std::string pwd = Config::GetInstance()->GetPwd();
	std::string bmpfile(pwd + "/certcard.bmp");
	cv::Mat  certcardMat = cv::imread(bmpfile);
	seeta::ImageData src_imgdata(certcardMat.cols, certcardMat.rows, certcardMat.channels(), certcardMat.data);
	seeta::ImageData dest_imgdata(frame.cols, frame.rows, frame.channels(), frame.data);
